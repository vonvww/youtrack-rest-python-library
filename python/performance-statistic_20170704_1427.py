...
def exportCSV(execdate, user, pwd, query, sprint):
    ...
    if datetime.now().strftime('%Y-%m-%d') == execdate:
        with open(path + sprint + '.csv', 'wb') as csvfile:
            ...
            spamwriter.writerow(
                ['sprint'] + ['姓名'] + ['绩效点数'] + ['调整值'] + ['调整原因'] + ['奖惩值'] + ['验证工时(分钟)'] + ['总工时(分钟)'] + [
                    '计算明细'] + [
                    '记录明细'])
        ...
        with open(path + sprint + '-DATE-' + everyday + '.csv', 'wb') as csvfile:
            csvfile.write(codecs.BOM_UTF8)
            spamwriter = csv.writer(csvfile, dialect='excel')
            spamwriter.writerow(
                ['sprint'] + ['姓名'] + ['绩效点数'] + ['调整值'] + ['调整原因'] + ['奖惩值'] + ['验证工时(分钟)'] + ['总工时(分钟)'] + ['计算明细'] + [
                    '记录明细'])
            allissue = yt.getAllIssues(query)
            verified = yt.getAllIssues('#Verified ' + query)
            statdic = {}
...
def main(args):
    ...
    if datetime.now().strftime('%Y-%m-%d') == execdate:
        ...
        if len(args) == 8:
            receivers = args[6]
            cc = args[7]
            sendMail('每天统计-'+sprint, receivers, cc, '执行时间: '+ date,path + sprint + '-DATE-' + everyday + '.csv')
