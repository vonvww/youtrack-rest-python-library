...
with open('../../'+ date + '_yourack.csv', 'wb') as csvfile:
    ...
    for issue in allissue:
        ...
        if len(keys) == 0:
            ...
            if not sstatdic.has_key(keys[0]):
                sstatdic[keys[0]] = {}
            sstatdic[keys[0]][issue.id] = es
        else:
            total = sum(ttdic.values())
            for k in keys:
                if not sstatdic.has_key(k):
                    sstatdic[k] = {}
                sstatdic[k][issue.id] = es * ttdic[k] / total
    ...
    for assignee in statdic:
        ...
        spamwriter.writerow([sprint,assignee,sd,'','',sh, ssh, task, detail])